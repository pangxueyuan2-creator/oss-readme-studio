const checks = [
  ...
  },
  {
    ...
    test: (markdown) => hasSection(markdown, ["installation", "install", "setup", "getting started", "run locally", "installation and setup"], true),
  },
  {
    id: "usage",
    label: "Usage example",
  ...
  },
  {
    ...
    test: (markdown) => hasSection(markdown, ["contributing", "contribution", "development", "contributing guidelines", "contribution guidelines"]),
  },
  {
    id: "license",
    label: "License",
...
function sectionBody(markdown, aliases) {
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      if (aliases.includes(title)) sectionLevel = level;
      continue;
    }
